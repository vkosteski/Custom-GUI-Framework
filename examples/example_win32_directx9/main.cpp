...
int main(int, char**)
{
    ...
    while (!done)
    {
        ...
        if ( !bg )
            D3DXCreateTextureFromFileExA( g_pd3dDevice, "C:\\Windows\\Web\\Screen\\img102.jpg", 1920, 1080, D3DX_DEFAULT, 0,
                D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &bg );

        blur::device = g_pd3dDevice;

        static bool bools[50]{};
        static int ints[50]{},ints2[50]{}, combo = 0;
        ...
        ImGui::Begin("Hello, world!", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground); {
            ...
            draw->AddText(io.Fonts->Fonts[1], io.Fonts->Fonts[1]->FontSize, pos + ImVec2(170 / 2 - io.Fonts->Fonts[1]->CalcTextSizeA(io.Fonts->Fonts[1]->FontSize, FLT_MAX, 0, "PAGY").x / 2 + 1, 20), gui.accent_color.to_im_color(), "NAME");
            draw->AddText(io.Fonts->Fonts[1], io.Fonts->Fonts[1]->FontSize, pos + ImVec2(170 / 2 - io.Fonts->Fonts[1]->CalcTextSizeA(io.Fonts->Fonts[1]->FontSize, FLT_MAX, 0, "PAGY").x / 2, 20), GetColorU32(ImGuiCol_Text), "NAME");
            ...
            draw->AddText(pos + ImVec2(50, size.y - 40), gui.text.to_im_color(), "PAGY");
            ...
            gui.group_title("Option 1");  // Changed from "Aimbot"
            if (gui.tab(ICON_FA_CROSSHAIRS, "Option A", gui.m_tab == 0) && gui.m_tab != 0)
                gui.m_tab = 0, gui.m_anim = 0.f;

            if (gui.tab(ICON_FA_GHOST, "Option B", gui.m_tab == 1) && gui.m_tab != 1)
                gui.m_tab = 1, gui.m_anim = 0.f;

            if (gui.tab(ICON_FA_MOUSE, "Option C", gui.m_tab == 2) && gui.m_tab != 2)
                gui.m_tab = 2, gui.m_anim = 0.f;

            Spacing(), Spacing(), Spacing();

            gui.group_title("Option 2");  // Changed from "Visuals"
            if (gui.tab(ICON_FA_USER, "Option D", gui.m_tab == 3) && gui.m_tab != 3)
                gui.m_tab = 3, gui.m_anim = 0.f;

            if (gui.tab(ICON_FA_PALLET, "Option E", gui.m_tab == 4) && gui.m_tab != 4)
                gui.m_tab = 4, gui.m_anim = 0.f;

            Spacing(), Spacing(), Spacing();

            gui.group_title("Option 3");  // Changed from "Miscellaneous"
            if (gui.tab(ICON_FA_HAMMER, "Option F", gui.m_tab == 5) && gui.m_tab != 5)
                gui.m_tab = 5, gui.m_anim = 0.f;

            if (gui.tab(ICON_FA_CODE, "Option G", gui.m_tab == 6) && gui.m_tab != 6)
            ...
            case 0:
                ...
                gui.group_box(ICON_FA_FEATHER " Options", ImVec2(GetWindowWidth() / 2 - GetStyle().ItemSpacing.x / 2, 210)); {

                    Checkbox("sheesh " ICON_FA_DAGGER, &bools[0]);
                    SliderInt("test", &ints[0], 0, 100, "%d%%");
                    SliderInt("test", &ints2[0], 0, 100, "%d%%");
                ...
                gui.group_box(ICON_FA_CROWN " King", ImVec2(GetWindowWidth() / 2 - GetStyle().ItemSpacing.x / 2, GetWindowHeight())); {
            ...
            case 1:
                ...
                gui.group_box(ICON_FA_BABY " Test", ImVec2(GetWindowWidth() / 2 - GetStyle().ItemSpacing.x / 2, GetWindowHeight() / 2 - GetStyle().ItemSpacing.y / 2)); {
